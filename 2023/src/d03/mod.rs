    ...
    for loc in uln.places_to_scan.iter() {
        ...
        for xmod in loc.x - 1..loc.x + 2 {
            for ymod in loc.y - 1..loc.y + 2 {
                ...
                match coords.get(&Coords { x: xmod, y: ymod }) {
                    Some(component_hit) => return Some(component_hit.clone()),
                    _ => (),
                }
            }
        }
    ...
    for line in input.lines() {
        ...
        for c in terminated_line.chars() {
            ...
            if digits.contains(&c) {
                ...
                places_to_scan.push(Coords { x, y })
            } else {
                if building_part_number {
                    unlinked_numbers.push(UnlinkedNumber {
                ...
                if c != '.' {
                    // Build a component
                    unlinked_components.push(UnlinkedComponent {
                        coordinates: ComponentCoordinates {
                            c: Coords { x, y },
                            symbol: c.to_string(),
                        },
...
pub fn fn2(input: &str) -> i64 {
    ...
    for component in linked.iter() {
        ...
        if ["*"]
            ...
            .contains(&component.coordinates.symbol)
        {
            if component.nearby_nums.len() == 2 {
                // LOOK AT THE EVIL 617. Called it!
                let mut st = 1;
                for part_number in component.nearby_nums.iter() {
                    st *= part_number
                }
